...
.widefat td, .widefat th {
	...
	<table class="widefat">
	    <thead>
			<tr><td colspan="200" style="text-align: center;"><?php _e('Quizzes Record', 'unilms-pro');?></td></tr>
		    <tr>
		        <th><strong><?php _e('Name-Roll No', 'unilms-pro');?></strong></th>
		        <?php 
		        	for ($i=1; $i <= $total_quizzes; $i++) { 
				?>
						<th><strong><?php printf(__('Quiz %s', 'unilms-pro'), $i);?></strong></th>
				<?php
					}
		        ?>
		    </tr>
	...
	?>
				<td><?php echo esc_html($quiz_marks) . ' '; ?><a href="<?php echo get_permalink(); ?>"><?php _e('View Quiz', 'unilms-pro');?></a></td>
	<?php
		    endwhile;
	?>
			</tr>
